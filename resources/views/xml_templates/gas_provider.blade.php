<root>
    <content>
        ...
        <provider id="{{ $apz->commission->apzGasResponse->user->id }}" type="gas">
            ...
            @if ($apz->commission->apzGasResponse->user->iin)
                <iin>{{ $apz->commission->apzGasResponse->user->iin }}</iin>
            @endif

            @if ($apz->commission->apzGasResponse->user->bin)
                <bin>{{ $apz->commission->apzGasResponse->user->bin }}</bin>
            @endif
        ...
        <provider_answer id="{{ $apz->commission->apzGasResponse->id }}" type="gas">
            @if($apz->commission->apzGasResponse->response)
                ...
                <assumed_capacity>{{ $apz->commission->apzGasResponse->assumed_capacity }}</assumed_capacity>
                <reconsideration>{{ $apz->commission->apzGasResponse->reconsideration }}</reconsideration>
            @else
                <response_text>{{ $apz->commission->apzGasResponse->response_text }}</response_text>
                <comments>{{ $apz->commission->apzGasResponse->comments }}</comments>
            @endif

            <doc_number>{{ $apz->commission->apzGasResponse->doc_number }}</doc_number>
            <response>{{ $apz->commission->apzGasResponse->response }}</response>
            <created_at>{{ $apz->commission->apzGasResponse->created_at }}</created_at>
        </provider_answer>
    </content>
</root>